			...
			if (cmd) {
				if (level >= cmd.conf.permLevel) {
					if (cmd.conf.enabled === true) {
						...
						message.reply('<:v2:773993754519404574> Este comando está desligado para manutenções');
					...
					message.reply('<:v2:773993754519404574> Você não tem permissão para isso!')
				...
				message.channel.send('<:v2:773993754519404574> Comando inexistente, digite `*help` para lista de comandos');
			}
		} else {
			cmd.run(client, message, args, level);
		}
